...
const Collaboration = () => {
  return (
    <Section crosses className="" id="collaboration" crossesOffset="" >
                    ...
                    <div className="flex w-60 aspect-square m-auto border border-n-6 rounded-full">
                        <div className='w-[6rem] aspect-square m-auto p-[0.2rem] bg-conic-gradient rounded-full'>
                            <div className="flex items-center justify-center w-full h-full bg-n-8 rounded-full">
                                <img src="/images/logo.png" alt="parallax" width={48} height={48} />
                            </div>
                        </div>
                    </div>
                    <ul>
                        {collabApps.map((app,index)=>(
                            <li key={app.id} className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${index * 45}`}>
                                <div className={`relative -top-[1.6rem] flex w-[3.2rem] h-[3.2rem] bg-n-7 border border-n-1/15 rounded-full -rotate-${index * 45}`}>
                                    <img className="m-auto" src={app.icon.src} alt={app.title} width={app.width} height={app.height} />
                                </div>
                            </li>
                        ))}
                    </ul>
